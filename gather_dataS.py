...
def annotate_data(FN):
    ...
    pc_targs.loc[:, 'GSMS'] = list(map(get_gsm, pc_targs.Names))
    ...
    wtc_targs.loc[:, 'GSMS'] = list(map(get_gsm, wtc_targs.Names))
...
def correlated_data_ecoli(z1_gene, NAME, GROWTH_ONLY=False, GROWTH_FN=''):
    if GROWTH_ONLY:
        annot_df = pa.read_excel('data/DataSetS1_CarreraAnnotations.xlsx',
                                sheet_name='EcoMAC and EcoPhe', index_col=u'CEL file name')
...
def main():
    ...
    if FN.endswith('pkl'):
        ...
        alldat = fill_nan(alldat)
    if FN.endswith('csv'):
        cols = alldat.columns.tolist()
        cols[0] = 'SystematicName'
        alldat.columns = cols
        alldat = alldat.set_index('SystematicName')
    if org == 'saccer':
